...
# redcode installer
# Usage: curl -fsSL https://raw.githubusercontent.com/kyxiaxiang/redcode/main/install.sh | bash
...
REPO="https://github.com/kyxiaxiang/redcode.git"
INSTALL_DIR="$HOME/redcode"
...
build_binary() {
  info "Building redcode (all experimental features enabled)..."
  cd "$INSTALL_DIR"
...
link_binary() {
  ...
  ln -sf "$INSTALL_DIR/cli-dev" "$link_dir/redcode"
  ok "Symlinked: $link_dir/redcode"
...
printf "    ${CYAN}redcode${RESET}                          # interactive REPL\n"
printf "    ${CYAN}redcode -p \"your prompt\"${RESET}          # one-shot mode\n"
echo ""
printf "  ${BOLD}Set your API key:${RESET}\n"
printf "    ${CYAN}export ANTHROPIC_API_KEY=\"sk-ant-...\"${RESET}\n"
echo ""
printf "  ${BOLD}Or log in with Claude.ai:${RESET}\n"
printf "    ${CYAN}redcode /login${RESET}\n"
echo ""
printf "  ${DIM}Source: $INSTALL_DIR${RESET}\n"
printf "  ${DIM}Binary: $INSTALL_DIR/cli-dev${RESET}\n"
printf "  ${DIM}Link:   ~/.local/bin/redcode${RESET}\n"
echo ""
